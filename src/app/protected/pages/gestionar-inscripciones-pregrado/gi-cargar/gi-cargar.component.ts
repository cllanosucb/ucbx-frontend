...
@Component({
    selector: 'app-gi-cargar',
    templateUrl: './gi-cargar.component.html',
    styleUrls: ['./gi-cargar.component.scss'],
})
export class GiCargarComponent implements OnInit {
    error: Error = {
        ok: false,
        error: {
    ...
    transformarDatosSemestre(semestres) {
        ...
        semestres.forEach((s) => {
            const l: Rsemestre = {
                ...
                name: `[${s.resumido} - ${s.descripcion}] - ${s.nombre_regional}`,
            };
            list.push(l);
        });
        return list;
    ...
    nextPage() {
        ...
        this.router.navigate([
            'index/gestionarInscripciones/pregrado/crearEstudiantes',
        ]);
    }
    ...
    subir(event) {
        ...
        this.cursosService.subirArchivoInscPregradoPrueba(formData).subscribe(
            (resp) => {
                this.success = resp;
        ...
        /* this.cursosService.subirArchivoInscPregrado(formData).subscribe(
            (resp) => {
                this.success = resp;
                this.successDatos = true;
                this.display = false;
